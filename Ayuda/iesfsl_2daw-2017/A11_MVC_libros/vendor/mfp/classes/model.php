...
abstract class model {
	...
	/**
	 * Convierte fecha de Base de datos a fecha visible
	 */
	...
	/**
	 * Convierte fecha visible a Base de datos
	 */
	...
	public function validate() {
		...
		foreach (static::$attributes as $atributo => $info) {
			if ($info['Extra'] != 'auto_increment') {
				if ($this->$atributo === "" or $this->$atributo === null) { // No tiene valor
	...
	static protected function getmetadata() {
		$metafile = 'cache/' . static::$tablename . '.des';
		if (!file_exists($metafile)) { //Busca en la caché
			...
			if (@!file_put_contents($metafile, serialize($attributes)))
				die("ERROR al escribir en la caché. Asigna permisos al directorio 'cache'");
		} else
			$attributes = unserialize(file_get_contents($metafile));
